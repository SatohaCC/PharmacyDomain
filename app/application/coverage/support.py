...
def parse_insured_type(raw: str | None) -> CoverageInsuredType | None:
    """入力文字列を本人・家族区分へ変換する。"""
    if raw is None:
        return None
    try:
        return CoverageInsuredType(raw)
    except ValueError as exc:
...
def build_insurance_details(
    *,
    ...
    return InsuranceCoverageDetails(
        ...
        benefit_ratio=(
            CoverageBenefitRatio(benefit_ratio) if benefit_ratio is not None else None
        ),
    )
