...
- **テナント境界**: コンテキストは `corporate` / `store` / `staff` / `patient` の4つ。集約間は **ID 参照のみ**（他集約のエンティティを直接保持しない）。他テナントデータへのアクセスは 403 ではなく 404（`XxxNotFoundError` または `TenantBoundaryNotFoundError`）として隠蔽する。
...
- **法人ライフサイクル**: `CorporateStatus.INACTIVE` の法人に対するStore / Staff / Patientの通常操作は `CorporateAccessService` で拒否し、状態変更はベンダーシステム管理者専用とする。Corporate自体には管理者ユーザーやロールを保持させない。
- **Applicationコンテキストの依存**: Store / Staff / Patient は法人コンテキストの実装を import しない。必要なのは「対象法人の認可と有効状態を確認して集約を得る」ことだけなので、`access_control` の `CorporateAccessBoundary`（Protocol）にだけ依存させる。実装 `CorporateAccessService` を import するのは Composition Root とテストのみ。依存は `patient` → `access_control`、`staff` → `store` → `access_control`、`corporate` → `access_control` の一方向で、逆向きの import は `tools/check_imports.py` が検出する。新しいコンテキストを追加するときは `[tool.import_rules.forbidden]` にも規則を足す。
