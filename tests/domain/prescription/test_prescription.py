...
``okf/ddd/prescription.md`` §5 の「守り手」列が ``Aggregate.validate()`` に
なっているものだけをここで固定する。Domain Service / Boundary が守るもの
...
class Test薬品補足と変更制限の振り分け:
    """処方編 別表16 を2つの型へ振り分けた結果の整合性（不変条件 #10）。"""
