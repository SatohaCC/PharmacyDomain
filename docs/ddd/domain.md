...
### コアドメインとレセコン（外部システム）の境界

本システムのコアドメインは **MedicationHistory（電子薬歴）**、**Dispensing（調剤）**、**Prescription（処方箋原本）**、**Patient（患者プロファイル）** です。
レセコン（医事会計・レセプト請求システム）の責務である調剤報酬点数算定、窓口一部負担金計算、公費自己負担上限額管理票の累積計算・記帳、レセプト作成はドメインモデルの関心事から明確に除外します。
レセコンからNSIPS連携等を介して届く調剤・処方データ（今回回数、合計分割回数、分割理由、調製方法等）は、集約内で算定要件を追試せず「確定した客観的業務事実」として受容します。
