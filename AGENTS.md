...
- **保存前の境界が読むものは、書く側と同じロックの内側で読む**: 境界が別集約を読み直して検証するなら、その集約を書くユースケースが取るのと同じキーで `OrganizationLock` を取ってから読む。取らないと、検証する側と書く側が互いの確定前を読んで**どちらも成功する**（資格を失ったスタッフが管理薬剤師のまま残り、例外は出ない）。ロックは対象の集約を保存するときだけ取り、`get()` では取らない（参照で取ると、参照が全ての書き込みと直列化する）。この競合はDBなしのテストでは再現しないので、`tests/integration/` の並行実行と、ロック取得が読み出しより前にあることを順序で固定するテストの両方で守る。
- **店舗の状態と業務**: 業務は `STORE_OPERATION_KINDS` で「新規業務 / 継続業務 / 参照」に分類し、新規業務は有効な店舗でだけ、継続業務は閉局していない店舗でだけ許す。薬歴は作成が新規業務、確定・訂正が継続業務（閉局で訂正まで止めると、記載漏れのある薬歴を直せないまま凍結する）。表のキーが列挙と一致することを検査するので、業務を足して区分を書き忘れると落ちる。**テストは判定の式を再現せず**、どの業務が新規業務かという分類を独立した表として持つ（期待値を実装から導くテストは、網羅していても実装と同時にしか壊れない）。
- **管理薬剤師の在任は店舗状態と別の軸**: 薬機法第7条の配置義務は、`MANAGER_REQUIRED_BY_KIND` が区分ごとに「在任を要するか」を宣言し、新規業務だけを在任日に限ることで守る。宣言は業務ごとではなく**区分ごと**に置く（業務ごとに書くと店舗状態の表と同じ値が並ぶ複製になり、片方だけ直した事故をどちらの表も検出できない）。在任は `StoreManagerAssignmentRepository.find_effective()` を業務日で引いて判定し、履歴を全件読んでから絞らない。拒否は `StoreStateConflictError` ではなく `ManagerAbsenceConflictError` として区別する（休止中の店舗と管理薬剤師のいない店舗では、利用者が次に取る手段が違う）。スタッフの配属と管理薬剤師の任命は `StoreOperation` に**載せない**。どちらも Domain Service が本物の集約で閉局を拒否しており、境界にも置くと判定が2箇所になる。実際 `assign_staff` / `assign_manager` は列挙にだけ存在して `require_allowed` を呼ぶ経路が無く、表の上でだけ守られているように見えていた。任命を在任の要る業務にすると最初の1人を任命できなくなる。
- **管理薬剤師の専任は自然人で判定する**: 薬機法第7条第3項の専任義務は法人内のスタッフではなく人にかかる。`StoreManagerAssignment` は `person_id` を持ち、排他制約の鍵を**法人IDを含めずに**人単位で張る（法人内のスタッフIDを鍵にすると、グループ内の別法人で同じ人物が両方の管理薬剤師になれる）。スタッフ単位の排他制約は置かない。複合外部キー `(staff_id, person_id) → staff_person_links(id, person_id)` によりスタッフから本人が一意に決まるので、人単位の制約が同一スタッフの重複も必ず捕らえる。両方を置くと、どちらの制約が報告されるかがサーバ任せになり業務例外の型が揺れる。この複合外部キーは「対応の無いスタッフの任命」と「対応とずれた本人の写し」も同じ制約で弾く。任命ユースケースは `StaffPersonBoundary` で本人を引き、未固定なら `ManagerPersonUnresolvedError` で拒否する（Identityのユースケースへは依存させない）。知事の許可による兼務の例外は表現しない。
- **開局時間は週次の予定と特例日で持つ**: `BusinessHours` は全7曜日の予定（時間帯が空なら定休日）と、日付を指定した特例（空なら臨時休業、非空なら臨時開局）からなる。**全曜日の宣言を必須**にし、書き漏らした曜日を定休日に倒さない。祝日の判定は内蔵しない（法改正で変わり、春分・秋分は毎年の天文計算で決まるので、暦を抱えるとドメインが毎年古くなる）。祝日は特例日として登録する。時間帯は**終了時刻を含まない半開区間**とする（資格や任命の期間は終了日を含む閉区間だが、時刻はここで向きが違う。昼休みを挟む 9:00-13:00 と 14:00-19:00 を閉区間にすると、13:00 と 14:00 が2つの時間帯にまたがる）。`time` は24時を表せないので、**終端に現れる `00:00` だけをその日の24時**と読む。日をまたぐ夜間帯は1件で持たず、その日の22-24時と翌日の0-2時に分ける。
- **開局しているかは3値で答える**: `Store.opening_state_at()` は `OPEN` / `CLOSED` / `UNKNOWN` を返す。開局時間が未登録のとき「開いている」に倒すと案内や電話対応が実在しない時刻を伝えるので、判定できないことを「該当しない」に倒さない。店舗状態が有効でなければ時間表に関わらず `CLOSED` とする（判定を状態と時間表の2箇所へ分けると、呼び出し側がどちらかを見落とす）。開局時間はJSONBのpayloadにだけ持ち、検索列を作らないので**マイグレーションは要らない**。既存行に項目が無くても dataclass の既定値へ落ちることを `tests/infrastructure/postgres/test_codec.py` が固定する。判定の日付と時刻は Domain が決めず、`app/application/common/clock.py` の `business_now(clock)` で業務のタイムゾーンへ揃えてから渡す。タイムゾーンの無い日時は受け取らない（UTCのつもりとJSTのつもりが同じ形で届き、9時間ずれた答えを黙って返す）。
- **閉局の取消は状態遷移ではなく訂正**: `Store.change_status` は閉局を終端のままにし、取消は `Store.revoke_closure()` という別の操作にする。閉局は廃止届を伴う手続きなので、通常の遷移として往復させない。戻す先は有効ではなく**休止**とする（閉局で管理薬剤師の任命は終了しているので、有効へ戻すと任命の無い店舗がその場で受付を通す）。任命は復元しない（閉局中に別店舗の管理薬剤師になった人と期間が重なりうる）。権限は `Permission.REVOKE_STORE_CLOSURE` でベンダーシステム管理者専用とし、判定はユースケースが持つ（ルータで先回りすると判定が2箇所になる）。
- **業務日とページング**: 業務日は `app/application/common/clock.py` の `business_date(clock)` だけが決める（`ZoneInfo("Asia/Tokyo")` を各所へ書かない）。カーソル付きの一覧は `app/application/common/pagination.py` の `Page[T]` を返す。`dict[str, object]` で返すと項目名も要素の型も伝わらず、OpenAPIにも自由形式のオブジェクトとしてしか載らない。
...
- **任意項目の変換はヘルパーへ寄せる**: `X.value if X is not None else None` を項目ごとに書かない。DTOの生成は `unwrap(X)`、入力からのプリミティブ生成は `build_optional(raw, Primitive)`（正規化も内包する）を使い、定義は `app/application/common/optional_conversion.py` に1つだけ置く。この定型は1行に収まらず折り返されるため、項目が増えるほどDTOのフィールド構成そのものが定型に埋もれる。さらに悪いことに、意味のある変換（`Decimal` を `str` へ落とす等）と単なる写しが同じ見た目になり、読み手が注目すべき箇所を見分けられなくなる。寄せたあとに残る条件式は、判定対象と取り出す対象がずれる**本物の条件**だけになる。`tests/application/common/test_optional_conversion.py` が `app/application` を走査し、両辺が同一の式である定型が再び書かれたら落とす。Application共通基盤は標準ライブラリだけに依存させる規則があるので、`DomainPrimitive` を直接受けず `value` を持つ構造的な型として受ける。
...
- **層の位置**: 永続化アダプタは `app/infrastructure/postgres/`、ユースケースの配線（Composition Root）は `app/infrastructure/di/` に置き、Domain / Application からは import しない。接続は `di/` が Protocol へ行う。逆向きの import は `tools/check_imports.py` が検出する。
- **Infrastructureのファイル構成**: `postgres/` は **SQLAlchemy に触る場所**に限る——`schema`（テーブル定義）、`connection`（接続・セッション・Unit of Work）、`codec`（集約とJSONBの変換）、`repository_base`（集約とテーブルの対応と読み書きの共通経路）、`repositories/`（Repository実装）。配線は `di/` へ分け、`root`（プロセスとリクエストの入口）、`registry`（コンテキスト束のレジストリ）、`bundles/`（束の実体）を置く。配線はDBの詳細を知らないのに `postgres` という名前のパッケージに同居していると、パッケージ名がその境界について嘘をつく。**Repositoryは1集約1ファイル**とし、ファイル名は Repository 名から `Postgres` と `Repository` を除いたものに揃える（集約から実装へ一意にたどれる）。集約は互いに独立して変わる（処方箋の検索列を足しても調剤・薬歴・頭書きは変わらない）ので、束ねてもファイルの中で区切り線を引き直すことになり、境界を探す手間だけが残る。**ユースケースの配線は逆に束のまま**にし、`di/bundles/` を `organization`（法人・店舗・スタッフ）/ `patient_care`（患者・外部識別子・資格・受付）/ `clinical`（処方箋・調剤・薬歴・頭書き）/ `medicine_catalog` の4つにまとめる。配線は `CorporateAccessService` / `Clock` / `UnitOfWork` という同じ依存を配り回しており、渡すものが増えると束の中の複数のユースケースが同時に変わるので、変更の単位が束と一致する。実際、配線を1コンテキスト1ファイルに割ったときは import が本文の1/4を占める30行台のファイルが並ぶだけになり、探す手間だけが増えた。`medicine_catalog` だけを単独の束に残すのは、法人IDを取らない唯一のコンテキストだからで、臨床の束へ混ぜるとその区別が消える。
- **ユースケース束はアクセス時に組み立てる**: `PostgresUseCaseRegistry` はプロパティで初めて該当コンテキストの束を作り、同一スコープ内でキャッシュする。1リクエストで使うのは通常1束なので、10束すべてを毎回作ると参照Boundaryの実アダプタまで無駄に生成される。リフレクション検査（配線の網羅性・ルートの網羅性）は束の一覧を型ヒントから読むので、束を足すときは `registry` の登録も併せて更新する。
- **1行1集約**: 集約は JSONB の `payload` 列を正とし、検索・一意性制約に必要な値だけを列へ複製する。読み込み時に列と payload の食い違いを検出して復元を拒否する（列だけ書き換えられた行を集約として通さない）。
- **列の導出は1度だけ書く**: 集約から検索列を導く処理は `app/infrastructure/postgres/repository_base.py` の `AggregateMapping.search_columns` にだけ置く。書き込み（`row_values`）も復元時の照合（`decode`）も同じ関数を使うので、片方だけ直して「保存はできるが読み戻せない行」を作れない。Repositoryごとに `row_values` と `_decode_row` を手で書くと、`None if x is None else x.value` のような導出が2箇所に現れ、食い違っても実DBに繋ぐまで誰も落ちない。`tests/infrastructure/postgres/test_schema_migration_consistency.py` が `repositories` パッケージの全 `AggregateMapping` を走査し、列の検査対象から漏れた集約があれば落とす。
- **集約の読み書きは基底経由に限る**: 読み取りは `PostgresRepositoryBase.find_one` / `find_all`、保存は `save_aggregate` を通す。行の世代（`version`）を記録する手続きと `upsert` は非公開にしてあり、Repositoryから直接は呼べない。公開すると、記録を忘れた読み取り経路だけ楽観ロックの期待値が空になり、その経路の保存が例外の出ない後勝ちの上書きになる。以前は `remember_version(cast(...), namespace=...)` を31箇所へ書き写しており、1箇所の漏れがその経路のロックだけを静かに外す形だった。
...
- **応答にドメインの語彙を載せない**: Application層のDTOはドメインプリミティブを保持してよいが、**応答モデルに使うDTO**は素の型（`str` など）へ落とす。`StoreId` をそのまま載せると本文に `{"value": "..."}` という入れ子が出て、生成クライアントは「IDは文字列ではない」という契約を受け取る。型検査もlintも通り、DBなしのテストも緑のままなので気づけない。任意のID参照は `app/application/common/optional_conversion.py` の`optional_id()` で落とす。`tests/presentational/test_response_shapes.py` が全ルートの応答モデルを再帰的にたどり、合併型・総称型・入れ子のdataclassの中まで検査する。
- **未知の項目は拒否する**: リクエスト本文は `RequestModel`（`extra="forbid"`）を継承させる。黙って捨てると、項目名を打ち間違えた変更要求が「成功したのに何も変わらない」応答になる。
- **開発用の認証は起動点ごと分ける**: 固定トークンで通す開発用の窓口は `app/presentational/dev_main.py` に置き、`create_dev_app()` を uvicorn の `--factory` から呼ぶ。`Dockerfile` の `CMD` は `app.main:app` のまま変えず、開発用の起動点を指すのは `compose.yaml` だけにする。危ないのは環境変数の付け忘れより**起動点の取り違え**なので、`tests/presentational/test_dev_main.py` が `Dockerfile` を読んで `dev_main` を指していないことを検査する。トークンは必須（未設定なら起動しない）で、既定値を持たせない。`dev_main` が読む `DEV_ACTOR_*` は `compose.yaml` と `.env.example` の両方へ載せる（渡し漏れは起動してみるまで分からない。`tests/presentational/test_dev_main.py` が実装の本文から変数名を集めて検査する）。「未設定なら通す」に倒すと開発用の起動点が無条件の穴になる。
- **エンジンはlifespanでだけ作る**: `create_app()` はモジュール読み込み時にDBへ触らない。読み込み時に作ると、`DATABASE_URL` が無い環境（テスト・OpenAPI生成）で import しただけで落ちる。トランザクションの開始と確定は `PostgresRequestScope` に任せ、ルータは `commit` も `rollback` も呼ばない。この「呼ばない」は規約ではなく規則で守る。`[tool.import_rules.forbidden]` が `app.presentational` から `sqlalchemy` を禁じるので、セッションやトランザクションAPIをルータへ持ち込んだ時点で落ちる。
