...
class AssignStaffConcurrentStoreCommand:
    """兼務店舗追加に必要な入力データ（DTO）。"""
...
class AssignStaffConcurrentStoreUseCase:
    """兼務店舗追加ユースケース。"""
    ...
    async def execute(self, command: AssignStaffConcurrentStoreCommand) -> None:
        ...
        updated_staff = self._assignment_service.assign_concurrent_store(
            staff,
            store,
            command.start_date,
        )
