...
（#3 回数 / #5 使用期間 / #6 次回予定日 / #8 変更制限 / #13 剤の対応）を固定する。
...
class Test変更制限:
    """不変条件 #8。薬剤師法第23条第2項（処方医の同意なき変更の禁止）。"""
...
class Test調剤回数と処方箋の指示:
    """不変条件 #3。分割理由ごとの上限は集約が見るので、ここでは処方箋側だけ。"""
