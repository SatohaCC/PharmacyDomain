...
async def test_同名の本人を登録しても一人へ統合しない() -> None:
    ...
    created = await fixture.service.register_person(
...
async def test_スタッフに一度対応させた本人を別人へ付け替えない() -> None:
    ...
    await fixture.service._staff.save(staff)
    link = await fixture.service.link_staff(
    ...
    with pytest.raises(IdentityConflictError):
        await fixture.service.link_staff(
...
async def test_店舗ロールの招待には同一法人で本人に対応する有効スタッフが必要(
    ...
    if violation == "退職":
        await fixture.service._staff.save(replace(staff, is_active=False))
    command = replace(
        fixture.command,
    ...
    with pytest.raises(
        ...
        await fixture.service.invite(command)
